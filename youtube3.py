...
class YouTubeLite:
    ...
    def extract(self, url_or_id):
        ...
        # =========== 【修复】新增：拦截并捕获新闻直播 HLS 流 ===========
        hls_url = None
        is_live = False
        for resp in responses:
            resp_streaming = (resp or {}).get('streamingData') or {}
            if 'hlsManifestUrl' in resp_streaming:
                hls_url = resp_streaming['hlsManifestUrl']
                is_live = True
                break
            resp_details = (resp or {}).get('videoDetails') or {}
            if resp_details.get('isLive') or resp_details.get('isLivePlayback'):
                is_live = True
        
        if is_live and hls_url:
            debug_log('detected live stream', {'hls_url': hls_url})
            live_details = player_response.get('videoDetails') or {}
            live_format = {
                'itag': 'live_hls',
                'url': hls_url,
            ...
            data = {
                'id': video_id,
                'title': live_details.get('title') or video_id,
                'duration': 0,
                'formats': [live_format],
                'is_live': True
            }
            self.extract_cache[video_id] = {'data': data, 'expires': time.time() + 60}
            return data
        # ============================================================
    ...
    def choose_playable(self, formats, quality=None):
        # =========== 【修复】新增：如果是直播流直接放行，不进行 Range 探测 ===========
        live_format = next((x for x in formats if x.get('vcodec') == 'hls' or x.get('ext') == 'm3u8'), None)
        if live_format:
            return live_format
        # =========================================================================
    ...
    def _extract_js_function_body(self, code, name):
        m = re.search(r'(?:function\s+' + re.escape(name) + r'|' + re.escape(name) + r'=function)\(([^)]*)\)\{([^}]+)\}', code)
        return m.group(2) if m else ''

    def _extract_helper_object(self, code, name):
        if not name:
            return {}
        m = re.search(r'var\s+' + re.escape(name) + r'=\{(.+?)\};', code, re.DOTALL)
        if not m:
            return {}
        body = m.group(1)
        helper_map = {}
        for part in body.split('},'):
            m2 = re.search(r'([a-zA-Z0-9_$]+):function', part)
            if not m2:
                continue
            key = m2.group(1)
            if 'reverse' in part:
                helper_map[key] = 'reverse'
            elif 'splice' in part or 'slice' in part:
                helper_map[key] = 'splice'
            else:
                helper_map[key] = 'swap'
        return helper_map
    # =========================================================================
...
class Spider(Spider):
    ...
    def init(self, extend=""):
        ...
        self.yt = YouTubeLite(self.session, headers=self.session.headers)
    ...
    def categoryContent(self, tid, pg, filter, extend):
        ...
        # 兼容性调用 YouTube 网关API搜索，这里输出TVBox标准的列表格式
        # （由于底层网关各平台封装不同，此处构造壳逻辑以保护流程完整）
        result['list'] = [] 
        result['page'] = pg
        result['pagecount'] = 99
        result['limit'] = 20
    ...
    def playerContent(self, flag, id, vipFlags):
        result = {}
        try:
            # 提权解析视频，若检测到新闻直播流，choose_playable 将自动无感秒级放行 M3U8 直播地址
            data = self.yt.extract(id)
            playable = self.yt.choose_playable(data['formats'], quality='1080p')
            if playable:
                result["parse"] = 0
                result["playUrl"] = ""
                result["url"] = playable['url']
                # 注入媒体请求所需的代理或特定UA头信息以保证IPTV正常加载
                if playable.get('headers'):
                    result["header"] = json.dumps(playable['headers'])
                if playable.get('ext') == 'm3u8' or playable.get('vcodec') == 'hls':
