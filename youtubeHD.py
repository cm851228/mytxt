...
class Spider(Spider):
    ...
    def init(self, extend):
        ...
        # ----- 从外部 JSON 读取代理配置，若无则使用默认内置代理 -----
        proxy_config = self.extendDict.get('proxy')
        default_proxy = 'http://127.0.0.1:10172'

        if proxy_config:
            if isinstance(proxy_config, str):
                if not proxy_config.startswith('http://') and not proxy_config.startswith('https://'):
                    proxy_config = 'http://' + proxy_config
                self.session.proxies = {
                    'http': proxy_config,
                    'https': proxy_config,
                }
                self.proxy_str = proxy_config.replace('http://', '').replace('https://', '')
                ...
                if proxies:
                    ...
                    self.proxy_str = (proxies.get('https') or proxies.get('http') or default_proxy).replace('http://', '').replace('https://', '')
                else:
                    self.session.proxies = {'http': default_proxy, 'https': default_proxy}
                    self.proxy_str = default_proxy.replace('http://', '')
            else:
                self.session.proxies = {'http': default_proxy, 'https': default_proxy}
                self.proxy_str = default_proxy.replace('http://', '')
        else:
    ...
    def _proxy_image(self, params):
        ...
        # 使用 i.ytimg.com 更稳定，也可用 img.youtube.com
