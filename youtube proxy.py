...
class YouTubeLite:
    ...
    def _client_audio_priority(self, client_name):
        # 数值越大越优先。ANDROID_VR/ANDROID 的 googlevideo 地址对 po_token 要求宽松，
        # 播放全程稳定；IOS/WEB/MWEB 未配置有效 po_token 时会在初始缓冲窗口（约1分钟）后
        # 对后续分段返回 403，导致音频断供、播放卡住。故优先选安全客户端。
        return {
            'ANDROID_VR': 4,
            'ANDROID': 3,
            'MWEB': 2,
    ...
    def choose_audio(self, formats):
        ...
        if not candidates:
            return None
        candidates.sort(key=lambda x: (self._client_audio_priority(x.get('client')), 1 if x.get('ext') == 'mp4' else 0, int(x.get('bitrate') or 0)), reverse=True)
