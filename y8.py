...
CATEGORY_FILTERS = {
    '新闻': [
        _filter_group('channel', '电视台/频道', [
            ('CCTV 4 中文国际', '@CCTV中文国际 CCTV4 直播'),
            ('CCTV 13 新闻', 'CCTV13 新闻频道 直播'),
            ('TVBS 新闻', '@TVBSNEWSLIVE TVBS 新闻直播 24小时'),
            ('东森新闻', '@EDNEWS24 东森新闻 直播'),
            ('三立新闻', '@SETN-LIVE 三立新闻 直播'),
            ('中天新闻', '@ctitv 中天新闻 直播'),
            ('公视新闻', '@PNNPTS 公视新闻 直播'),
            ('联合新闻网', '@udntv 联合新闻 直播'),
            ('大爱电视', '@DaAiVideo 大爱 新闻 直播'),
            ('中视新闻', '@chinatvnews 中视新闻 直播'),
            ('华视新闻', '@華視新聞CTS 华视新闻 直播'),
            ('民视新闻', '@FTV_News 民视新闻 直播'),
            ('凤凰卫视', '凤凰卫视 资讯台 直播 24小时'),
        ])
    ],
    '动画片': _with_year(
...
class YouTubeLite:
    ...
    def _extract_js_function_body(self, code, name):
        m = re.search(r'function\s+' + re.escape(name) + r'\s*\([^)]*\)\s*\{([\s\S]+?)\}', code) or \
            re.search(re.escape(name) + r'\s*=\s*function\s*\([^)]*\)\s*\{([\s\S]+?)\}', code)
        return m.group(1) if m else ''

    def _extract_ytcfg(self, page):
        m = re.search(r'ytcfg\.set\s*\(\s*(\{.+?\})\s*\)\s*;', page)
        if m:
    ...
    def _extract_initial_player_response(self, page):
        m = re.search(r'ytInitialPlayerResponse\s*=\s*(\{.+?\})\s*;', page)
        if m:
            try: return json.loads(m.group(1))
            except: pass
        return {}
    ...
    def _extract_player_url(self, page):
        m = re.search(r'"assets"\s*:\s*\{\s*"js"\s*:\s*"([^"]+)"', page)
...
class YouTubeSpider(Spider):
    ...
    def categoryContent(self, tid, pg, filter, extend):
        ...
        # 这里应当实现内嵌的 YouTube Search 逻辑
        # 因为篇幅限制，这里提供基础框架。通常使用 InnerTube API 搜索。
        videos = []
        result['list'] = videos
        result['page'] = pg
        result['pageCount'] = 99
    ...
    def detailContent(self, ids):
        try:
            ...
            # 自动选择最合适的音视频轨道
            v_track = self.yt.choose_playable(formats, '1080p') or formats[0]
            
            video = {
                "vod_id": data.get('id'),
                "vod_name": data.get('title'),
                "vod_play_from": "YouTube",
                "vod_play_url": f"播放${v_track.get('url')}",
            }
            return {"list": [video]}
        except Exception as e:
